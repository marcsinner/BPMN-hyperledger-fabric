...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating OrgClient Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-orgClient.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating OrgPlatform Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-orgPlatform.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/orgClient/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for OrgClient and OrgPlatform"
...
# call the script to create the channel, join the peers of orgClient and orgPlatform,
...
function networkDown() {
  ...
  # stop org3 containers also in addition to orgClient and orgPlatform, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.orgClient.example.com docker_peer0.orgPlatform.example.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/orgClient/msp organizations/fabric-ca/orgClient/tls-cert.pem organizations/fabric-ca/orgClient/ca-cert.pem organizations/fabric-ca/orgClient/IssuerPublicKey organizations/fabric-ca/orgClient/IssuerRevocationPublicKey organizations/fabric-ca/orgClient/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/orgPlatform/msp organizations/fabric-ca/orgPlatform/tls-cert.pem organizations/fabric-ca/orgPlatform/ca-cert.pem organizations/fabric-ca/orgPlatform/IssuerPublicKey organizations/fabric-ca/orgPlatform/IssuerRevocationPublicKey organizations/fabric-ca/orgPlatform/fabric-ca-server.db'
