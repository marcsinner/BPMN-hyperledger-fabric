...
// Colours used in terminal
const C_RED='\x1b[31m%s\x1b[0m'
const C_GREEN='\x1b[32m%s\x1b[0m'
const C_BLUE='\x1b[34m%s\x1b[0m'
const C_YELLOW='\x1b[33m%s\x1b[0m'
...
async function main() {
    ...
    try {
        ...
        // Return all the current assets on the ledger that correspond to the object.
        await getAllAssetsByObject(contract);
        // Check the availability of the asset in the given pincode area.
        await getAssetAvailabilityByPincode(contract);
...
async function transferAssetAsync(contract) {
    ...
    console.log(C_GREEN, `*** Successfully submitted transaction to transfer ownership from ${oldOwner} to Saptha`);
...
/*async function updateNonExistentAsset(contract) {
    console.log('\n--> Submit Transaction: UpdateAsset asset70, asset70 does not exist and should return an error', 'background: #ccffcc; color: #ff4dd2');
    try {
...
/*async function createAsset(contract) {
    console.log('\n--> Submit Transaction: CreateAsset, creates new asset with ID, Color, Size, Owner and AppraisedValue arguments', 'background: #ccffcc; color: #ff4dd2');
    await contract.submitTransaction('CreateAsset', assetId, 'yellow', '[80809]', 'Tom');
    console.log('*** Transaction committed successfully');
}*/
